...
def test_kkt_components_hand_computed():
    ...
    G = jnp.array([[1.0, -1.0]])
    h = jnp.array([0.0])
    lp = create_lp(c, A, b, G, h, jnp.zeros(2), 10.0 * jnp.ones(2),
    ...
    prim, dual, pobj, dobj = compute_kkt_components(
        lp, x, y, Ax, ATy, qx, jnp.inf
    )
    ...
    lp_free = create_lp(c, A, b, G, h,
                        -jnp.inf * jnp.ones(2), jnp.inf * jnp.ones(2),
                        use_sparse_matrix=False)
    y2 = jnp.array([0.0, 3.0])
    ATy2 = jnp.array([3.0, -3.0])
    prim2, dual2, _, _ = compute_kkt_components(
        lp_free, x, y2, Ax, ATy2, qx, jnp.inf
    )
